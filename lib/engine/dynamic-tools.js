...
function syncDynamicTools(reqTools, session, compilerTools, reverseMap) {
  ...
  for (const key of Object.keys(compilerTools)) {
    if (!inbuiltGeneric.has(key)) {
      ...
      console.warn('[DynamicTools] Removing previously registered dynamic tool:', key)
    }
  }
  ...
  // Cache grammar in-memory on session (not persisted — reconstructed on hash match)
  session._dynamicGrammarCache = dynamicGrammar

  console.log(`[DynamicTools] synced ${grammarLines.length} MCP tool(s) (hash changed)`)
