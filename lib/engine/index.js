...
const { decodeContentParts, uploadFiles } = require('../../utils/extract-files')
 ...
 * response — shared by emitSkill/emitTitle to bypass the provider API.
 *
 * @param {import('express').Response} res
 * @param {Stream} parser - already-constructed Stream for this response
 * @param {string} text - text to scan and emit
 */
ToolCompiler.emitText = function (res, parser, text) {
  parser.scan(text)
  parser.flush()
  parser.emit({}, 'stop', {})
  res.write('data: [DONE]\n\n')
...
/**
 * Emit a matched skill's predefined BPI as synthetic tool_calls and end
 * the response — used to bypass the provider API when a skill matches.
 *
 * @param {import('express').Response} res
 * @param {Stream} parser - already-constructed Stream for this response
 * @param {object} skill - matched skill object { triggers, bpi }
 */
ToolCompiler.emitSkill = function (res, parser, skill) {
  ToolCompiler.emitText(res, parser, skill.bpi)
}

/**
 * Emit a synthetic title-completion for OpenCode's title-generation utility
 * call — bypasses the provider entirely, echoing back the session's own
 * human-assigned name. No session state (chatSessionId/parentMessageId/
 * lastUsed), no rate-limit slot, no network round-trip.
 *
 * @param {import('express').Response} res
 * @param {Stream} parser - already-constructed Stream for this response
 * @param {string} title - text to emit as the title (session.name)
 */
ToolCompiler.emitTitle = function (res, parser, title) {
  ToolCompiler.emitText(res, parser, title || 'New session')
}
