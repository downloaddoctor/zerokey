...
class ToolCompiler {
  ...
  constructor(ideName, provider = 'chatgpt') {
    ...
    const { tools, user, tool, rawUser, reverseMap } = getIDEMapper(ideName)
    ...
    this._handlers = {
      ...
      tool: async (mes, messages, _is, uploadFile) => {
        ...
        return `BPI(${name}): ${truncate(tool(name, mes.content))}`
      },
    }
  ...
  async formatPrompt(messages, isNewSession, uploadFile) {
    ...
    if (message?.role === 'user' && this._rawUser) {
      try {
        ...
        skill = ToolCompiler.matchSkill(text)
      } catch (e) {
...
/**
 * Find the skill whose triggers include the given raw user text
 * (already trimmed/lowercased by the caller). O(1) map lookup.
 *
 * @param {string} text
 * @returns {object|null}
 */
ToolCompiler.matchSkill = function (text) {
  if (!text) return null
  return skillsByTrigger.get(text) || null
}
