...
class ToolCompiler {
  ...
  constructor(ideName, provider = 'chatgpt') {
    ...
    function toolFormatter(messages, lastIdx = 1) {
      const toolResults = []
      for (let i = messages.length - lastIdx; i >= 0; i--) {
        const m = messages[i]
      ...
      return toolResults
        .map((m) => {
          const name = getGenericToolName(m.tool_call_id)
          return `BPI(${name}): ${truncate(tool(name, m.content))}`
        })
        .join('\n\n')
    }
    ...
    this._handlers = {
      ...
      user: (c, messages, isNewSession) => {
        let skipped_tools = toolFormatter(messages, 2)
        if (skipped_tools) {
          skipped_tools += '\n\n'
        }

        return skipped_tools + user('USER: ', c, messages, isNewSession)
      },
      tool: (c, messages) => {
        return toolFormatter(messages, 1)
      },
    }
