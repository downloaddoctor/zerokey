...
const IDES_PROMPT_OPTIMIZER = {
  vscode: {
    user: (prefix, content, messages, isNewSession) => {
      ...
      let userText = current.userRequest?.content || content || ''
      const mes = []
      if (messages.length == NEW_SESSION_START_LENGTH.vscode || isNewSession) {
        console.info('\n\n[SESSION] NEW STARTED!')
      ...
      if (current.attachments?.content?.length) {
        mes.push(current.attachments.full)
      }
...
function shortenToolOutput(name, output) {
  ...
  s = s.replace('The user cancelled the tool call.\n', '[CANCELLED BY USER]')
