...
const TOOLS = {
  read: {
    desc: 'Read file contents by absolute path. Optional offset (0-based start line) and limit (max lines to return).',
  ...
  write: {
    desc: 'Create a new file at path with full string content. Overwrites if exists',
  ...
  // Edit Tools
  edit: {
    desc: 'Safest block replacer. `before`/`after` are fences — they stay untouched. Only code between them changes. CRITICAL: Make each anchor UNIQUE by including enough surrounding context (newlines, indentation, adjacent code) until the string appears exactly ONCE in the target region. → To replace line X: set `before`=line-above-X and `after`=line-below-X. Never use X as `before`. Single chars like `}` are NEVER valid anchors — include the line before/after it.',
    grammar: 'path={str}|before={str:UNIQUE}|after={str:UNIQUE}|content={str}',
    keys: { path: ' ', before: ' ', after: ' ', content: ' ' },
    eg: [
      {
    ...
    transformer: (params) =>
      applyTransform(params, 'EDIT', (content) => {
        const newText = (params.content || '').replaceAll('\\n', '\n')

        const before = resolveAnchors(content, params.before, 'EDIT')
        if (before.idx === -1) {
          params.old = params.before
          return newText
        }

        const after = resolveAnchors(content.substring(before.idx), params.after, 'EDIT')
        if (after.idx === -1) {
          params.old = params.after
          return newText
        }

        params.old = content.slice(before.idx, before.idx + after.idx + after.resolved.length)
  ...
  append: {
    desc: 'Inserts after `anchor` line. No anchor → end of file.',
  ...
  prepend: {
    desc: 'Inserts before `anchor` line. No anchor → start of file.',
    grammar: 'path={str}|(anchor={str})?|content={str}',
    keys: { path: ' ', anchor: ' ', content: ' ' },
    eg: [
      {
  ...
  replace: {
    desc: 'Swap exact old→new. old must be unique in file.',
  ...
  list: {
    desc: 'List immediate children (files and subdirs) of a directory path. Subdirs end with /.',
    grammar: 'path={str}',
  ...
  mkdir: {
    desc: 'Create a new directory, recursively creating missing parents (like mkdir -p).',
    grammar: 'path={str}',
  ...
  glob: {
    desc: 'Find files matching a glob pattern (e.g. **/*.js, src/**\/*.ts). Optional max caps result count.',
  ...
  grep: {
    desc: 'Search file contents with plain text or regex. Set regex=true when query is a regex pattern. Optional pattern filters files by glob, max caps results.',
  ...
  cmd: {
    // desc: 'Run a shell command. goal is a short label (10-50 chars). Optional desc adds context, till sets timeout in seconds.',
    // grammar: 'run={str}|goal={str:10-50}|(desc={str:20-100})?|(till={0-300})?',
    desc: 'Run a shell command. till sets timeout in seconds.',
  ...
  todo: {
    desc: 'Create or update a structured task list. Batch multiple entries with id=Ntitle=Tstatus=S. status: wait | active | done. Use only when 3+ tool calls are needed.',
