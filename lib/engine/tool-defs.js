...
const TOOLS = {
    ...
    transformer: (params) => {
      const resolveByLineNo = (entry, filePath) => {
        ...
        if (from < 0) {
          ...
          console.log(`[replaceByLineNo] from<=0 → prepend mode`)
          return
        }
        ...
        if (from >= lines.length) {
          ...
          console.log(`[replaceByLineNo] out-of-bounds → append mode`)
          return
        }
        ...
        console.log(`[replaceByLineNo] lines ${entry.from}-${entry.to} → ${entry.old.length} chars`)
      }
  ...
  grep: {
    ...
    keys: { query: ' ', regex: true, pattern: ' ', max: 200 },
    vscode: {
      tool: 'grep_search',
      params: {
        query: 'query',
    ...
    opencode: {
      tool: 'grep',
      params: { query: 'pattern', pattern: 'include' },
      default: { pattern: ' ' },
    },
  },
  cmd: {
