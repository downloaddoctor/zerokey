...
function resolveAnchors(content, anchor, label) {
  ...
  if (idx === -1) throw new Error(`[${label}] Anchor not found - ${anchor}`)
  if (content.indexOf(resolved, idx + 1) !== -1)
    console.warn(`[${label}] Anchor appears multiple times, use more context to make it unique`)
...
const TOOLS = {
  ...
  edit: {
    desc: 'Boundaries stay, between replaced. TRAP: content = body ONLY. Never include first/last — causes duplication.',
    grammar: 'path={str}|first={str}|last={str}|content={str}',
    keys: { path: ' ', first: ' ', last: ' ', content: ' ' },
    eg: [
      {
        path: '/path/to/file.txt',
    ...
    transformer: (params) =>
      applyTransform(params, 'EDIT', (content) => {
        const first = resolveAnchors(content, params.first, 'EDIT')
        const last = resolveAnchors(content.substring(first.idx), params.last, 'EDIT')
        const newText = (params.content || '').replaceAll('\\n', '\n')

        params.old = content.slice(first.idx, first.idx + last.idx + last.resolved.length)
  ...
  append: {
    desc: 'Add after anchor. No anchor → end of file.',
  ...
  prepend: {
    desc: 'Add before anchor. No anchor → start of file.',
    grammar: 'path={str}|(anchor={str})?|content={str}',
    keys: { path: ' ', anchor: ' ', content: ' ' },
    eg: [
      {
...
const EDIT_TOOLS_PROMPT =
  "\n# EDIT TOOLS\n- `replace` for 1-2 lines, `edit` for 2+ lines. \n- edit's content = Never include boundaries → duplication error.\n\n"
