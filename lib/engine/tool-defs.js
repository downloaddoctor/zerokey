...
const SHORTENERS = {
  ...
  'todo+': (s) => (s.startsWith('Successfully wrote todo list') ? 'UPDATED' : s),
  'todo!': (s) => (s.startsWith('Successfully wrote todo list') ? 'UPDATED' : s),
  read: (s) => {
  ...
  cmd: (s) => {
    ...
    if (s.startsWith('Large tool result ')) {
      ...
      if (filePath) return `[LARGE OUTPUT] read → ${filePath[1]}`
    }
    if (s.startsWith('[Output compressed'))
      return s
        ...
        .replace(/\s*Note:.*$/, '')
        .trim()
    if (s.startsWith('Note: This terminal execution was moved to the background using the ID')) {
      const idMatch = s.match(/using the ID\s+(\S+)/)
      return idMatch
        ? `[BACKGROUND] ${idMatch[1]} — timed out, will notify on completion.`
        : '[BACKGROUND] Command timed out — will notify on completion.'
    }
    if (s.startsWith('Note: The tool simplified the command to')) {
      let [_, ...output] = s.split('\n')
      return output.join('\n')
    }
