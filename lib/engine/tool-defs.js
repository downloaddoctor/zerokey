    ...
    transformer: (params) => {
      const resolveByLineNo = (entry, filePath) => {
        ...
        if (from < 0) {
          ...
          console.log(`[replaceOp] from<=0 → prepend mode`)
          return
        }
        ...
        if (from >= lines.length) {
          ...
          console.log(`[replaceOp] out-of-bounds → append mode`)
          return
        }
        ...
        console.log(`[replaceOp] lines ${entry.from}-${entry.to} → ${entry.old.length} chars`)
      }
